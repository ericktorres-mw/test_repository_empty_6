...
import { fetchVendorItems } from "../functions";
...
function createVendorItemsSublist(form, vendorId) {
  try {
    ...
    const itemResults = fetchVendorItems(vendorId);
    ...
    const itemsJson = JSON.stringify(itemResults);

    const vendorItemsField = form.addField({
