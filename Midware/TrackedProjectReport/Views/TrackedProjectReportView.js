 ...
 */
define(["require", "exports", "N/ui/serverWidget", "N/runtime", "../Constants/Constants", "N/ui/message"], function (require, exports, serverWidget, runtime, constants, message) {
    Object.defineProperty(exports, "__esModule", { value: true });
    function getMainView() {
        var form = serverWidget.createForm({ title: "Tracked Project Name Report" });
    ...
    function getResultView(tracked_project_id, transacData, communicationData) {
        var form = serverWidget.createForm({ title: "Tracked Project Name Report" });
        form.addSubmitButton({ label: "Search" });
        form.addField({
            id: constants.SUITELET_IDS.FORM.FIELDS.PROJECT_SELECT.ID,
    ...
    function buildSOSubList(form, transacData) {
    ...
    function setSOSubListData(transactionSubList, transacData) {
        var line = 0;
        for (var i = 0; i < transacData.length; i++) {
            var transac = transacData[i];
            if (transac["transacType"] == "SalesOrd") {
                var fields = transac["fields"];
    ...
    function buildQuotesSubList(form, transacData) {
        var transactionSubList = form.addSublist({
    ...
    function setQuotesSubListData(transactionSubList, transacData) {
        var line = 0;
        for (var i = 0; i < transacData.length; i++) {
            var transac = transacData[i];
    ...
    function buildCommunicationSubList(form, communicationData) {
    ...
    function setCommunicationSubListData(communicationSubList, communicationData) {
        var env = String(runtime.envType);
        for (var i = 0; i < communicationData.length; i++) {
            var communic = communicationData[i];
            var fields = communic["fields"];
            communicationSubList.setSublistValue({
                id: constants.SUITELET_IDS.COMMUNICATION_SUBLIST.VIEW,
                value: "https://1233958" + (env === "SANDBOX" ? "-sb1" : "") + ".app.netsuite.com/app/crm/" + communic["record"] + ".nl?id=" + communic["internalId"],
                line: i,
            });
            communicationSubList.setSublistValue({
                id: constants.SUITELET_IDS.COMMUNICATION_SUBLIST.COMMUNICATION_DATE,
